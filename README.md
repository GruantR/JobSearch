...
4. **Переменные окружения:** в корне проекта создайте файл **`.env.development`** (удобнее для разработки) или **`.env`**. Загрузка идёт через [dotenv-flow](https://github.com/kerimdzhanov/dotenv-flow); при запуске без `NODE_ENV` используется режим development, как у `npm run dev`.
   ...
   **Администратор:** после миграций при старте сервера выполняется сидер: если заданы **`ADMIN_EMAIL`** и **`ADMIN_PASSWORD`**, создаётся пользователь с ролью `admin` или существующий с таким email получает роль `admin`. Без этих переменных сидер пропускается (остаются только обычные регистрации с ролью `user`). Эндпоинт `GET /users/all` доступен только с JWT пользователя с ролью `admin`.
...
| `ADMIN_EMAIL`, `ADMIN_PASSWORD` | Нет | Вместе задают первого админа при старте (сидер после миграций) |
| `PUBLIC_URL` / `WEBHOOK_BASE_URL` / `RENDER_EXTERNAL_URL` | Нет | Продакшен: HTTPS-URL сервиса для Telegram webhook (Render + бот) |
...
## Render + Neon + Telegram-бот

- **Neon** — только PostgreSQL: задайте **`DATABASE_URL`** в переменных окружения Render (как в Neon).
- **Бот** в продакшене работает через **webhook** на тот же процесс, что и API (не polling), если задан публичный URL:
  - **`PUBLIC_URL`** — например `https://your-service.onrender.com` (без слэша в конце), или
  - **`RENDER_EXTERNAL_URL`** — Render может подставить URL сервиса (добавьте переменную вручную в Dashboard, если нужно).
- **`ENABLE_BOT=false`** — полностью отключает бота (тогда **`TELEGRAM_BOT_TOKEN`** не обязателен при старте).
- После деплоя проверьте в логах Render, что webhook зарегистрирован; при смене URL перезапустите сервис или вызовите `setWebHook` снова (перезапуск приложения достаточно).
